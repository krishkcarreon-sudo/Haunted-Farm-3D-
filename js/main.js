...
function buildSilo(){
  ...
  // Gas can (red metal can propped against west wall — only while not yet collected)
  if(!gasCanCollected){
    scene.add(makeBox(.28,.46,.22,0xcc2200,-8.8,.23,-5));           // can body
    scene.add(makeBox(.32,.06,.26,0xdd3311,-8.8,.49,-5));           // shoulder step
    scene.add(makeBox(.08,.24,.08,0xcc2200,-8.8,.62,-5));           // neck
    scene.add(makeBox(.14,.06,.12,0xaaaaaa,-8.8,.74,-5));           // cap
    scene.add(makeBox(.04,.14,.04,0x888888,-8.65,.76,-5));          // nozzle
    scene.add(makeBox(.20,.02,.14,0xff6644,-8.8,.02,-5));           // base shadow
    addLabel('F = GAS CAN \u26fd', -8.8, 1.3, -5, '#ff8844');
  }
...
function buildGeneratorRoom(){
  ...
  if(hasGasCan){
    addLabel('F = REFUEL GENERATOR \u26fd', 1.72, 1.5, -6, '#ff8844');
  } else if(power<=5){
    addLabel('NEED GAS CAN — check SILO', 1.72, 1.5, -6, '#ff4400');
  } else {
...
function doInteract(){
  ...
  // Gas can pickup (silo, west wall)
  if(currentSpot==='silo' && !gasCanCollected){
    var gcdx=-8.8-playerPos.x, gcdz=-5.0-playerPos.z;
    if(Math.sqrt(gcdx*gcdx+gcdz*gcdz)<2.8){
      gasCanCollected=true; hasGasCan=true;
      showMsg('\u26fd Gas can found! Take it to the GENERATOR ROOM (Closet \u2192 S door).', 3800);
      buildRoom(); return;
    }
  }

  // Generator refuel (generator room, fuel intake port at x=1.72, z=-6)
  if(currentSpot==='generator_room' && hasGasCan){
    var grdx=1.72-playerPos.x, grdz=-6.0-playerPos.z;
    if(Math.sqrt(grdx*grdx+grdz*grdz)<3.0){
      hasGasCan=false; power=100;
      ...
      showMsg('\u26a1 GENERATOR REFUELED! Power restored to 100%!', 3500);
      buildRoom(); return;
    }
  }
...
function animate(){
  ...
  var speed=(crouching ? MOVE_SPEED*0.45 : MOVE_SPEED)*dt;
  ...
  playerPos.y = crouching ? 0.85 : 1.6;
  ...
  for(var ek in enemyMeshes){
    ...
    var ms=ebot.name==='FARMER'?(farmerPhase===3?2.2:(farmerPhase===2?1.7:.95)):
           (ebot.name.indexOf('PLUSH')>=0?.3:(ebot.cage?1.2:(isSprinter&&hasSight?1.8:(ebot.big?1.5:.6))));
    ...
    if(dist<1.8){
      var dmg=ebot.name==='FARMER'?(farmerPhase===3?4.0:(farmerPhase===2?2.5:1.0)):
              (ebot.name.indexOf('PLUSH')>=0?.22:(ebot.cage?.90:(ebot.big?1.12:.45)));
